# streamlit_app.py (VERSIONE DEFINITIVA E CORRETTA)
    ...
    if not all([ticker, strike_price, premium]):
        ...
        with st.spinner(f"Recupero dati e calcolo in corso per {ticker}..."):
            ...
            if stock_data is None or stock_data.empty:
                ...
                # Assicuriamo di estrarre un singolo valore float, non una Series
                ...
                try:
                    ...
                    if pd.isna(implied_vol):
                        ...
                        st.info("**Interpretazione:** Il grafico mostra l'andamento della volatilità realizzata (storica) del sottostante su diverse finestre temporali...", icon="💡")

                except ValueError as e:
                    if "ArbitrageError" in str(e):
                        min_price = max(0.0, current_stock_price - strike_price)
                        st.error(f"**Errore di Pricing Rilevato!** Il premio inserito è inferiore al valore intrinseco. Controlla i dati.", icon="🚨")
                    else:
